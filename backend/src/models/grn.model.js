...
class GrnModel {
    ...
    create = async ({grn_no,grn_date,po_id,created_on,created_by,status,refno,remarks}) => {
        const sql = `INSERT INTO ${this.tableName} 
        (grn_no,grn_date,po_id,created_on,created_by,status,refno,remarks}) VALUES (?,?,?,?,?,?,?,?)`;
        console.log(sql);
        const result = await query(sql, [grn_no,grn_date,po_id,created_on,created_by,status,refno,remarks]);
...
update = async (params, id) => {
    ...
    const sql = `UPDATE grn SET ${columnSet} WHERE id = ?`;
